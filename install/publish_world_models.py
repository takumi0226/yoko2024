...
class WorldPublisher:
    def __init__(self):
        ...
        try:
            ...
            rclpy.wait_for_service('/gazebo/get_model_state', 100)
            self._get_model_state = rclpy.ServiceProxy('/gazebo/get_model_state', GetModelState)
        except rclpy.ROSException:
            print('/gazebo/get_model_state service is unavailable')
            sys.exit()

        if rclpy.has_param('~meshes'):
            meshes = rclpy.get_param('~meshes')
            if type(meshes) != dict:
                raise rclpy.ROSException('A list of mesh filenames is required')

            self.add_meshes(meshes)

        self._mesh_topic = rclpy.Publisher('/world_models', MarkerArray, queue_size=1)
...
if __name__ == '__main__':
    ...
    rclpy.init_node('publish_world_models')
